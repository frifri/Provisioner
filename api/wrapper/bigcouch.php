...
class wrapper_bigcouch {
    ...
    */
    ...
    public function add($database, $document) {
        ...
        try {
            $created_doc = $this->_couch_client->storeDoc($document);
            return $created_doc;
        } catch (Exception $e) {
            return false;
        } 
    ...
    public function get_provider($provider_domain) {
        ...
        $couch_client = new couchClient($this->_server_url, $providers_db);

        try {
            $response = $couch_client->key($provider_domain)->asArray()->getView($providers_db, 'list_by_domain');
